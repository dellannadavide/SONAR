...
// internal commands, I execute them immediately
+to_shut_down
<- .shut_down(internal);
    +is_shut_down;
    -to_shut_down.
...
//Note: sometimes the in belief said(Person, <something>), <something> has actually been generated by the data collector.
//When the "something" comes from the human, the words are separated by "__" (which corresponds to the constant Contstants.ASL_STRING_SEPARATOR
//When the "something" comes from the data collector, instead, the words are separated by "_" (so they are actually just commands, obtained from what the person actually said)
//this distinction is not necessary, but it helps distinguishing what comes from what
//in general some things come from the data collector because the human can say many different things for the same concept, so we translate them in a single one

// Shut down
+!reason_and_act_about_commands:
    said(Person, shut__down) & //note a human command
...
+!reason_and_act_about_commands:
    ...
    not prohibited_goal(tell_beliefs)
<- -said(Person, tell__me__what__you__know);
    !tell_beliefs(tellbel).

+!reason_and_act_about_commands:
    said(Person, tell__me__what__you__know) &
    prohibited_goal(tell_beliefs)
<- -said(Person, tell__me__what__you__know).
...
+!reason_and_act_about_commands:
    said(Person, tell_robot_name) & //note a command from the data collector
    not prohibited_goal(tell_robot_name)
<- -said(Person, tell_robot_name);
    !tell_robot_name.

+!reason_and_act_about_commands:
    said(Person, tell_robot_name) &
    prohibited_goal(tell_robot_name)
<- -said(Person, tell_robot_name).
...
+!reason_and_act_about_commands:
    said(Person, tell_what_user_said) &
    not prohibited_goal(tell_what_user_said)
<- -said(Person, tell_what_user_said);
    !tell_what_user_said.

+!reason_and_act_about_commands:
    said(Person, tell_what_user_said) &
    prohibited_goal(tell_what_user_said)
<- -said(Person, tell_what_user_said).
...
+!reason_and_act_about_commands:
    said(Person, tell_what_you_said) &
    not prohibited_goal(tell_what_you_said)
<- -said(Person, tell_what_you_said);
    !tell_what_you_said.

+!reason_and_act_about_commands:
    said(Person, tell_what_you_said) &
    prohibited_goal(tell_what_you_said)
<- -said(Person, tell_what_you_said).
...
+!start(spontaneous_conversation) <- true.
...
+!reason_and_act_about_speech:
    said(Person, Something)
 <- .ignore_last_person_said(Person, Something);
    -said(Person, Something).
...
+!reason_and_act_about_spontaneous_speech <- true.

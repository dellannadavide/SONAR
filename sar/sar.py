...
class SARBDIAgent(Agent):
    ...
    def __init__(self, jid: str, password: str, verify_security: bool = False, gui_queue = None):
    ...
    async def setup(self):
        ...
        self.fsi = FuzzySocialInterpreter(fuzzy_sets_file, ling_vars_file, rules_file, 0.2) #this is an intraagent element, sort of a database
        self.fsq = {}
        ...
        if "sys_handler" in workers_to_start:
            time.sleep(1)
        ...
        if "chatter" in workers_to_start:
            time.sleep(1)
        ...
        if "position_handler" in workers_to_start:
            time.sleep(1)
        ...
        if "vision_handler" in workers_to_start:
            time.sleep(1)
        ...
        if "posture_handler" in workers_to_start:
            time.sleep(1)
        ...
        if "collector" in workers_to_start:
            time.sleep(1)
        ...
        if "norm_adapter" in workers_to_start:
            time.sleep(1)

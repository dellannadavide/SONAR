...
def getDissimilaritySKFuzzyMF(umf1, mf1, umf2, mf2):
    """
...
def createSKFuzzyMFandGetDissimilarity(universe_boundaries, mf1_params, mf2_params):
    umfs = np.arange(universe_boundaries[0], universe_boundaries[1] + 0.000001, 0.05)
    mf1 = fuzz.trapmf(umfs, list(mf1_params[k] for k in ('a', 'b', 'c', 'd')))
    mf2 = fuzz.trapmf(umfs, list(mf2_params[k] for k in ('a', 'b', 'c', 'd')))
    return getDissimilaritySKFuzzyMF(umfs, mf1, umfs, mf2)
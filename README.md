...
- **Chorégraphe Suite**. Useful for creating and controlling behaviors for Nao; for installing behaviors on the robot; and for having access to the virtual robot.
...
NOSAR makes use of the Python library [SPADE](https://spade-mas.readthedocs.io/en/latest/readme.html), which is a Multi-agent platform based on [XMPP](https://xmpp.org/) (the open universal messaging standard). SPADE requires access to an XMPP communication server. Below, it is reported a quick tutorial on how to install [Prosody](https://prosody.im/), the SPADE recommended XMPP server on Windows 10. For more details See the [SPADE quick start](https://spade-mas.readthedocs.io/en/latest/usage.html) and the [Prosody guidelines](https://prosody.im/download/start#windows).
...
>2. Run ``` wsl --install ```
>3. Reboot the system 
>4. Open Microsoft Store and get Ubuntu
...
>Steps
    ...
    2. That's it. See Section **!!!!!TO DECIDE!!!!!** for details on how to create accounts for the SPADE agents.
...
Specifically concerning the python MQTT-Nao-Interface, such an interface runs on Python 2.7 (which is not well supported anymore as of 2022). Decoupling such an interface from the implementation of NOSAR, facilitates the implementation of several modern features that rely on libraries based on Python 3.x.
...
>Steps
    ...
    1. To install Mosquitto, run ``` sudo apt install mosquitto ``` and type ```Y``` when requested.
...
These behaviors can be found in folder ```MQTT-Nao-Interface.lib.Nao_Additional_Behaviors```, which contains a Choreograph project.
...
>3. In the ```Robot applications``` panel in Choreograph press the button ```Package and install current project to the robot```. An entry named ```Nao_Additional_Behaviors``` should appear in the list of applications of the robot.
>4. Run, on Choreograph, one of the behaviors (e.g., open ```behaviors/anger/behavior.xar``` and press play after connecting to the (virtual) robot). By doing so all behaviors in the project will be also automatically loaded in the ```.lastUploadedChoreographeBehavior``` application, which is the only application available to the virtual robot.
...
Additionally, as per Section, NOSAR relies on XMPP and MQTT services. Follow steps in Section 2 to install them.
...
> 4. The library makes extensive use of pretrained language models, which will be downloaded (if not provided already) at the first execution. The whole set of models might require a couple of GB of space on the hard drive.
...
The first step required in order to allow communication between agents in teh NOSAR is to register the agents in the XMPP server with a ```jid``` and a ```password```.
...
> **Example.** Assuming that you installed the Prosody XMPP server locally by simply following the steps indicated in Section 2.1, the server IP will be by default ```localhost```.
...
The list of required agents and XMPP-related information (e.g., name, server, jid, pwd) can be found at the bottom of file ```nosar.utils.constants.py```. All agents in the ```XMPP_AGENTS_DETAILS``` data structure should be registered in the xmpp server.

...
def submitJob(index_list, allNames, results_folder, all_experiments_timestamp, verbose, c_list):
    ...
    for c in c_list:
        ...
        command_concat_str = command_concat_str + ("" if (command_concat_str == "") else " &" ) + "python ./run_experiment_adaptation_commandline.py "+str(exp_param_args)
        exp_ind = exp_ind + 1

    # p = subprocess.Popen('python', stdout=subprocess.PIPE, stdin=subprocess.PIPE, shell=False)
...
def main():
    ...
    exp_possible_val = {
        ...
        'dataset_file': ['data_USAustria_adaptation_experiments_ds1_1k_new.xlsx',
                         'data_USAustria_adaptation_experiments_ds2_1k_new.xlsx',
                         'data_USAustria_adaptation_experiments_ds3_1k_new.xlsx',
                         'data_USAustria_adaptation_experiments_ds4_1k_new.xlsx',
                         'data_USAustria_adaptation_experiments_ds5_1k_new.xlsx'],
        'max_dataset_size': [1000],  # [1000, 8000, 16000], #[1000, 999999],
        'fuzzy_sets_file': ['fuzzy_sets_multiple_ref.xlsx'],
        'ling_vars_file': ['ling_var_multiple_ref_sim.xlsx'],
        'rules_file_id': ['_ref_all_refined3'], #['_ref_all_refined2'], # ['_ref_all'], ['_ref_all_refined']
